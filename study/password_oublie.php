...
if($_SERVER['REQUEST_METHOD']=="POST"){
	...
	if(filter_var($_POST["email"],FILTER_VALIDATE_EMAIL)){
		...
		if($_POST["token"]==$_SESSION["csrf_token"]){
			...
			$infosUser=$users->infosUser($_POST["email"]);

			if($infosUser){
				$connexion=$users->codePassword($_POST["email"],$code);
				...
				$envoieEmail=$users->envoiesMail($_POST["email"],$message);

				//header("location:design/home.html");
          ...
          <div class="modal-content rounded-4 shadow">
            ...
            <div style="background: transparent;" class="modal-body p-5 pt-0">
              <form method="post" class="">
                <h2 style="text-align: center; margin-top: 4%; font-size: 32px; font-weight: bold;">Mot de passe oublié</h2>
                <p class="titre-form">Remplissez le formulaire pour réinitialiser <br> votre mot de passe</p>
                <div class="form-floating mb-3">
                  ...
                  <button class="w-100 mb-2 btn btn-lg rounded-3 btn-primary" type="submit">Envoyer le code de réinitialisation</button>
              </form>
            </div>
          </div>
        </div>
