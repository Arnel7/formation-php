...
if($_SERVER['REQUEST_METHOD']=="POST"){
	...
	if(filter_var($_POST["email"],FILTER_VALIDATE_EMAIL)){
		...
		if($_POST["token"]==$_SESSION["csrf_token"]){
			$users=new Users($bdd);
			$verification=$users->infosUser($_POST["email"]);
			
			
			if($verification){
					//ici on ne fait rien

					$erreur="Votre email existe deja ";
			...
			$users->inscription($nom,$_POST["email"],$password);
...
<body>
    ...
    <div style="margin-top: 10%;" class="modal modal-signin position-static d-block   " tabindex="-1" role="dialog" id="modalSignin" >
        <div class="modal-dialog" role="document">
          <div class="modal-content rounded-4 shadow">
            
            <div style="background: transparent;" class="modal-body p-5 pt-0">
              <form method="post"  class="">
                <h2 style="text-align: center; margin-top: 4%; font-size: 32px; font-weight: bold;">S'inscrire</h2>
                <p class="titre-form">Vous avez déjà un compte? <a style="text-decoration: none;" href="login.html">S'identifier</a></p>
				<h3><?php   if(isset($erreur)){echo $erreur;}  ?></h3>
				<div class="form-floating mb-3">
					<input type="text" required name="nom" class="form-control rounded-3" id="floatingPassword" placeholder="Nom">
                ...
                <div class="form-floating mb-3">
                  <input type="password" name="password" required  class="form-control rounded-3" id="floatingPassword" placeholder="Mot de passe">
                  <label for="floatingPassword">Mot de passe</label>
						<input type="hidden" name="token" value="<?php echo $_SESSION["csrf_token"]; ?>">

                </div>
                <div style="display: flex;" class="com">
                    <input class="form-check-input" type="checkbox" value="" id="flexCheckDefault">
                  <label style="margin-left: 5px;" class="form-check-label" for="flexCheckDefault">
                      J'accepte les <a style="text-decoration: none; margin-bottom: 2%;" href="#">conditions d'utilisation</a> et <a style="text-decoration: none; margin-bottom: 2%;" href="#">la politique de confidentialité.</a>
                  </label>
                </div>
